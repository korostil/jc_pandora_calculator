...
#
#         assert json.loads(cache.get(user_id)) if cache.get(user_id) else cache.get(user_id) == expected_state
#
#
# @pytest.mark.parametrize('source', ['vk'])
# @pytest.mark.parametrize(
...
#         ('Lots (20-49)', {'status': 'inquire_for_town', 'guard_number': 'Lots (20-49)'})
#     ]
# )
...
#     [
#         ({'text': 'test', 'attachments': []}, {'text': 'test', 'screenshot': None}, None),
#         (
#                 {'attachments': [{'photo': {'sizes': [{'type': 'x', 'url': 'http://vk.com'}]}}]},
#                 {'text': '', 'screenshot': 'http://vk.com'},
#                 None
#         ),
#         (
#                 {'attachments': [{'photo': {'sizes': [{'type': 'x', 'url': 'http://vk.com'}]}}, {'video': {}}]},
#                 {'text': '', 'screenshot': 'http://vk.com'},
...
#     cache.set(user_id, '{"status": "inquire_for_town", "guard_number": "Lots (20-49)", "screenshot": "/tmp/test.png"}')
#     if source == 'vk':
#         conversation = VKConversation(user_id)
#
#         conversation.finish()
